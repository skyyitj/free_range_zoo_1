...
The wildfireGPT domain simulates a grid-based environment where fires can spread and agents are tasked with extinguishing
...
- **agent openness**: Environments where agents can dynamically enter and leave, enabling dynamic cooperation and
  multi-agent scenarios with evolving participants.
    - `wildfireGPT`: Agents can run out of suppressant and leave the environment, removing their contributions
...
- **task openness**: Tasks can be introduced or removed from the environment, allowing for flexbile goal setting
  and adaptable planning models
    - `wildfireGPT`: Fires can spread beyond their original starting point, requiring agents to reason about new
...
- **frame / type openness**: Different frames (e.g. agent abilities or skills) can be added, removed, or modified,
  expending the environmental complexity and requiring agents to infer their neighbors changing abilities.
    - `wildfireGPT`: Agents can damage their equipment over time, and have their capabilities slowly degrade. On
...
| Import             | `from free_range_zoo.envs import wildfireGPT_v0`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
from free_range_zoo.envs.wildfireGPT.env import transitions
from free_range_zoo.envs.wildfireGPT.env.utils import in_range_check
from free_range_zoo.envs.wildfireGPT.env.spaces import actions, observations
from free_range_zoo.envs.wildfireGPT.env.structures.state import WildfireState


def parallel_env(wrappers: List[Callable] = [], **kwargs) -> BatchedAECEnv:
    """
    Paralellized version of the wildfireGPT environment.

    Args:
        wrappers: List[Callable[[BatchedAECEnv], BatchedAECEnv]] - the wrappers to apply to the environment
    Returns:
        BatchedAECEnv: the parallelized wildfireGPT environment
    """
    env = raw_env(**kwargs)
...
def env(wrappers: List[Callable], **kwargs) -> BatchedAECEnv:
    """
    AEC wrapped version of the wildfireGPT environment.

    Args:
        wrappers: List[Callable[[BatchedAECEnv], BatchedAECEnv]] - the wrappers to apply to the environment
    Returns:
        BatchedAECEnv: the AEC wrapped wildfireGPT environment
    """
    env = raw_env(**kwargs)
    env = OrderEnforcingWrapper(env)
...
class raw_env(BatchedAECEnv):
    """Environment definition for the wildfireGPT environment."""

    metadata = {"render.modes": ["human", "rgb_array"], "name": "wildfireGPT_v0", "is_parallelizable": True, "render_fps": 2}
    ...
    def step_environment(self) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], Dict[str, Dict[str, bool]]]:
        """
        The actual simultaneous action wildfireGPT environment step
        """
        # Initialize storages
        ...
        # Assign rewards
        # # 调用 Eureka 生成的新奖励函数
        # rewards, reward_info = self.compute_reward(self._state, self.actions)
        #
        # # 保留原环境终止条件判断（但不再给奖励，奖励由compute_reward负责）
        ...
        #
        # # 更新终止条件字典
        # terminations = {agent: batch_is_dead for agent in self.agents}
    ...
    @torch.no_grad()
    def compute_reward(self, state: WildfireState, actions: Dict[str, torch.Tensor]) -> Tuple[
        Dict[str, torch.Tensor], Dict[str, Dict[str, Any]]]:
        """
        Compute the reward given the current state and agents' actions.

        Args:
            state: WildfireState - current environment state
            actions: Dict[str, torch.Tensor] - actions taken by agents

        Returns:
            Tuple[Dict[str, torch.Tensor], Dict[str, Dict[str, Any]]] - rewards and additional info per agent
        """
        # Eureka-generated reward will be inserted here
        pass
...
# Eureka-generated reward function
