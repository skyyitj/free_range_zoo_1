...
def main(cfg):
    ...
    # UGVUAV_ROOT_DIR = os.environ['UGVUAV_ROOT_DIR']
    ...
    # env_parent = 'cliport'
    env_parent = 'wildfire'
    task_file = f'{FRZ_ROOT}/envs/{env_parent}/env/{env_name}.py'
    ...
    exp_dir = f"{FRZ_ROOT}/experiments/{env_name}_eureka"
    os.makedirs(exp_dir, exist_ok=True)
    output_file = f"{exp_dir}/{env_name}{suffix.lower()}.py"
    # output_file = f"{UGVUAV_ROOT_DIR}/env/{env_name}{suffix.lower()}.py"
    ...
    for iter in range(cfg.iteration):
        ...
        while True:
            ...
            for attempt in range(1000):
                try:
                    logging.info(f"Iteration {iter}: GPT System Input:\n " + messages[0]["content"] + "\n")
                    logging.info(f"Iteration {iter}: GPT User Input:\n " + messages[1]["content"] + "\n")
                    kwargs = {
                        "mode": model,
                        "messages": messages,
        ...
        for response_id in range(cfg.sample):
            ...
            for i, line in enumerate(lines):
                if line.strip().startswith("def "):
                    code_string = ("\n" + short_indent).join(lines[i:])
            print('================ code_string =================')
            code_string = short_indent + code_string
            print(code_string)
            ...
            # ]
            reward_signature = [
                f"self.rew_buf[:], self.rew_dict = self.{gpt_reward_signature}",
                f"self.extras['gpt_reward'] = self.rew_buf.mean()",
                f"for rew_state in self.rew_dict: self.extras[rew_state] = self.rew_dict[rew_state].mean()",
            ]
            reward_signature = "\n".join([indent + line for line in reward_signature])
            print('================ reward_signature =================')
            print(reward_signature)
            ...
            if "def compute_reward(self)" in task_code_string:
                ...
                task_code_string_iter = task_code_string
            #     # task_code_string_iter = task_code_string.replace("def compute_reward(self, actions):", f"{gpt_reward_signature}:\n" + reward_signature)
            ...
            #                                 stdout=f, stderr=f)
            # with open(rl_filepath, 'w') as f:
            #     process = subprocess.Popen(['python', f'{UGVUAV_ROOT_DIR}/methods/IPPO_uav/train.py',
            #                                 'hydra/output=subprocess',
            #                                 f'method_name=IPPO_uav', f'agent=ppo',
            #                                 f'seed=1', f'device=0',
            #                                 f'algo=d3pm_sac', f'ugv_move_cost=true', 'uav_n=4',
            #                                 f'ugv_n=4', f'ugv_move_policy=random', f'uav_loc_obs_channel_num=4',
            #                                 f'update_ugv=false'],
            #                                stdout=f, stderr=f)
            with open(rl_filepath, 'w') as f:
        ...
        for response_id, (code_run, rl_run) in enumerate(zip(code_runs, rl_runs)):
            ...
            if traceback_msg == '':
                ...
                if not tensorboard_logdir:
                    logging.error("No tensorboard directory found in output")
                    return None

                if not os.path.exists(tensorboard_logdir):
                    logging.error(f"Tensorboard directory does not exist: {tensorboard_logdir}")
                    return None
                print(f"tensorboard_logdir = '{tensorboard_logdir}'")
