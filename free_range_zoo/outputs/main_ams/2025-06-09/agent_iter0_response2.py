...
def single_agent_policy(
    ...
    for task_index in range(num_tasks):
        ...
        # Task score is a combination of negative distance, potential suppression, and its importance weight
        # Define temperature parameters for scaling the influence of each term
        distance_temp = 2.0
        effect_temp = 1.0
        weight_temp = 0.5
        
        # Score computation using exponential weighting to normalize terms
        task_score = (
            -np.exp(distance / distance_temp) +  # We want smaller distances, hence, inverse relation
