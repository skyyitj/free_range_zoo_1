        ...
        for batch in range(self.parallel_envs):
            ...
            for index in range(self.argmax_store[batch].size(0)):
                if [index, 0] in action_space.spaces[0].enumerate():
                    ...
                    beta = 0.1
                    alpha = 1
                    gamma = 0.1  # proximity weight
                    score = (2 / ((alpha * distance + 1) * (beta * intensity + 4))) + (gamma / (distance + 1))
                    if score > max_score:
                        max_score = score
                        max_index = index
            if max_index == -1:
            ...
            self.actions[batch, 1] = 0 if has_suppressant[batch] else -1
        return self.actions
