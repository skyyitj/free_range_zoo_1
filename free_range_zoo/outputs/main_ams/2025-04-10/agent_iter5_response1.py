...
def single_agent_policy(
    ...
    # Prioritize suppression on the most intense fires
    if agent_suppressant_num >= fire_intensities[best_fire_idx]:
        return best_fire_idx
    else:
        # Consider partial suppression on high-intensity fires
        # Filter fires that require less suppressant than available
        viable_fires = [i for i in active_fires if fire_intensities[i] <= agent_suppressant_num]
        
        # If there are multiple viable fires, prioritize the one with the highest intensity per suppressant
        best_fire_idx = max(viable_fires, key=lambda i: fire_intensities[i] / agent_suppressant_num)
        return best_fire_idx