def single_agent_policy(
    ...
    # Example decision-making process for which fire to address (basic logic)
    max_fire_level = max(fire_levels)
    if max_fire_level > 5:  # if any fire level is above 5, pick the fire with the highest intensity
        fire_index = fire_intensities.index(max(fire_intensities))
    else:
        fire_index = fire_levels.index(max(fire_levels))
