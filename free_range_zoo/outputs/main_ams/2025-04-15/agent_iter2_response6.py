def single_agent_policy(
    ...
    for task in range(num_tasks):
        ...
        # Check if agent has sufficient resources to fully extinguish the fire
        if fire_intensities[task] <= can_put_out_fire:
            # Increase the score if the agent can completely extinguish the fire
            score *= 1.5

        scores[task] = score * fire_putout_weight[task]
