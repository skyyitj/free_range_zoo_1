def single_agent_policy(
    ...
    # Introduce strategic adjustment by considering suppressant conservation
    suppressant_residual = agent_suppressant_num - potential_fire_reductions
    
    # Increase conservation value if suppressant remains better otherwise lower it for balanced usage
    suppressant_conservation = suppressant_residual / agent_suppressant_num

    # Combine all scores to determine which fire to target
    # Incorporate fire importance, suppressant effectiveness directly, desperation inversely
    scores = normalized_distances * np.array(fire_putout_weight) * suppressant_effectiveness * fire_desperation * suppressant_conservation
