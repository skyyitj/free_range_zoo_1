def single_agent_policy(
    ...
    max_score = -float('inf')
    best_fire = None
    temperature = 0.03  # lowering temperature to emphasize the score difference.
    ...
    for i, (fire_position, fire_level, fire_intensity, fire_weight) in enumerate(zip(fire_pos, fire_levels, fire_intensities, fire_putout_weight)):
        ...
        # Adding fire_level to the score, to avoid the fire burning out due to constant high fire level
        score = np.exp((fire_weight * suppression_power / (dist + 1) + 1.5 *  fire_level) / temperature) # increasing weight for fire level
