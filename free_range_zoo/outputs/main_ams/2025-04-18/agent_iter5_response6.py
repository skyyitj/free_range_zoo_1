def single_agent_policy(
    ...
    max_score = float('-inf')
    best_fire = None
    temperature = 0.02  # Decrease the temperature to make differences in scores more significant
    ...
    for i, (fire_position, fire_level, fire_intensity, fire_weight) in enumerate(zip(fire_pos, fire_levels, fire_intensities, fire_putout_weight)):
        ...
        # Multiply the fire level by 3 to place more emphasis on fires with higher levels
        score = np.exp((fire_weight * suppression_power / (dist + 1) + 3 * fire_level) / temperature)
