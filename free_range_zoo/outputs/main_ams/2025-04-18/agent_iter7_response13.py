def single_agent_policy(
    ...
    intensity_temperature = 0.3  # Adjust temperature coefficient to balance intensity factor
    ...
    for i, (fire_position, fire_level, fire_intensity, fire_weight) in enumerate(zip(fire_pos, fire_levels, fire_intensities, fire_putout_weight)):
        ...
        # Intensity factor with medium weight
        intensity = (fire_level+1) / (fire_intensity+1)

        # Score calculation considering prioritization weight, firefighting efficiency, and distance factor
        score = np.exp((fire_weight * suppression_power / (dist_temperature * dist + 1) + fire_level) / suppress_power_temperature) * np.exp(intensity / intensity_temperature)
