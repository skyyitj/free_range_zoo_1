def single_agent_policy(
    ...
    max_score = -float('inf')
    best_fire = None
    temperature = 0.03  # Further lower temperature to get the decision difference more clear

    for i, (fire_position, fire_level, fire_intensity, fire_weight) in enumerate(zip(fire_pos, fire_levels, fire_intensities, fire_putout_weight)):

        # Distance factor (increased denominator by adding more fire level for distance factor)
        dist = ((fire_position[0]-agent_pos[0])**2 + (fire_position[1]-agent_pos[1])**2)**0.5 / (agent_suppressant_num + fire_level + 1)
