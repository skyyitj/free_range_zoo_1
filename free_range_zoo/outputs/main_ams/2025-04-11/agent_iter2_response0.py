def single_agent_policy(
    ...
    for task_id in range(num_fires):
        # Calculate required suppression power for each fire
        required_power = fire_intensities[task_id] * distances[task_id]
        
        # Only proceed if the agent's power is larger than required power
        if agent_fire_reduction_power > required_power:
            # Calculate closeness factor (inverse of the distance)
            closeness_factor = 1 / distances[task_id]

            # Calculate the efficiency score of current task with closeness factor applied
