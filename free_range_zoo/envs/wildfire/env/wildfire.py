...
class raw_env(BatchedAECEnv):
    ...
    def step_environment(self) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], Dict[str, Dict[str, bool]]]:
        ...
        # print('self._state.fires:', self._state.fires)
        ...
        infos['rewards'] = {}
        infos['rewards']['Burning Number'] = fire_positions.sum(dim=(-2, -1)).item()
        infos['rewards']['Putout Number'] = just_put_out.sum(dim=(-2, -1)).item()
        infos['rewards']['Burnedout Number'] = just_burned_out.sum(dim=(-2, -1)).item()
        ...
        for agent in self.agents:
            ...
            # infos[agent]['rewards'] = fire_rewards_per_batch
            # infos[agent]['num_burnouts'] = self.num_burnouts
        ...
        # print('termination_reward:', self.reward_config.termination_reward)
        ...
        for agent in self.agents:
            ...
            infos[agent]['termination_reward'] = termination_reward[newly_terminated]
