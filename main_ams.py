...
def main(cfg):
    ...
    for iter in range(cfg.iteration):
        ...
        for response_id in range(cfg.sample):
            ...
            # 精确匹配函数定义
            policy_pattern = r'def single_agent_policy$.*?$:\n.*?(?=\n\S|\Z)'
            match = re.search(policy_pattern, response_cur, re.DOTALL)

            # 如果没有匹配到，尝试匹配代码块
            if not match:
                code_block = re.search(r'```python(.*?)```', response_cur, re.DOTALL)
                if code_block:
                    code_content = code_block.group(1).strip()
                    # 再次尝试从代码块中匹配
                    match = re.search(policy_pattern, code_content, re.DOTALL)

            # 生成agent文件
            if match:
                function_code = match.group(0).strip()
                # 替换现有文件中的函数
                function_filename = f"agent_iter{iter}_response{response_id}.py"
                filepath = os.path.join(target_dir, function_filename)
                with open(filepath, "w") as f:
                    f.write(function_code)
                agent_files.append(function_filename)

                with open(GENERATED_AGENT_PATH, 'r') as f:
                    original_code = f.read()

                # 替换函数实现
                modified_code = re.sub(
                    r'def single_agent_policy$.*?$:\n.*?(?=\n\S|\Z)',
                    function_code,
                    original_code,
                    flags=re.DOTALL,
                    count=1
                ...
                # 保存修改后的文件
                with open(GENERATED_AGENT_PATH, 'w') as f:
